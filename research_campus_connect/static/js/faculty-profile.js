...
async function loadFacultyProfile() {
    ...
    try {
        ...
        document.getElementById('loading').textContent = 'Error loading faculty profile';
    }
}
...
function displayFacultyProfile(faculty) {
    ...
    document.getElementById('faculty-designation').textContent = faculty.designation;
    ...
    document.getElementById('faculty-phone').textContent = faculty.phone || 'Not provided';
    ...
    const researchAreas = faculty.research_areas.split(',').map(area => area.trim());
    researchAreasContainer.innerHTML = researchAreas.map(area => 
        `<span class="research-area">${area}</span>`
    ).join('');
